...
app = Celery('hello', broker='sqs://'+sqsId+':'+sqsPass+'@sqs.us-east-2.amazonaws.com//celery')
app.conf.update(
...
def cypressHeadless(id):
        ...
        try:
            ...
            subprocess.check_call('rm -rf cypress/screenshots/*', shell=True)
            ...
            client = boto3.client('s3', aws_access_key_id=awsAccessKeyId, aws_secret_access_key=awsSecretAccessKey)
        ...
        save_images(id, '/home/ubuntu/cypress/cypress/screenshots/'+str(idScript)+'_spec.js/')
        return 4
...
def monkeyHeadless(id):
        ...
        try:
            ...
            print('Ejecutando Script')
            subprocess.check_call('rm -rf /home/ubuntu/gremlins-webdriver/errorShots/*', shell=True)
        ...
        save_images(id, '/home/ubuntu/gremlins-webdriver/errorShots/')
        return 4
...
def connectSelect(query):
    ...
    try:
        ...
        conn = psycopg2.connect(host="smarttoolsdb.cqbimrfyvxj0.us-east-2.rds.amazonaws.com",database="tsdc_automationdb", user=sqlUser, password=sqlPass)
        cur = conn.cursor()
        cur.execute(query)
        data = cur.fetchall()
